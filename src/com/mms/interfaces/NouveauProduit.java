...
public class NouveauProduit extends JDialog {
	...
	public NouveauProduit() {
		...
		panelLabel.add(new JLabel("Qté initiale :"));
		panelLabel.add(new JLabel("Qté idéale :"));
		panelLabel.add(new JLabel("Qté securité :"));
		...
		if (!f_designation.getText().equals("")
			...
			if (Integer.parseInt(f_prixUnitaireAchat.getText()) > Integer
				...
				if (categorieService.creerCategorie(
					...
					/** Le produit courant est deja dans la base de données ***/
					JOptionPane.showMessageDialog(null,
							"Ce produit existe déjà !!!", "Erreur",
							JOptionPane.ERROR_MESSAGE);
				}
			}
		} else {
			/** Si tous es champs ne sont pas renseignés ***/
			JOptionPane.showMessageDialog(null,
