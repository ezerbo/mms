...
public class PasswordUpdateUI extends JDialog {
	...
	public void changerMotDePasse(){
		Utilisateur loggedInUser = UserService.getLoggedInUser();
		if(!f_motDePasse.getText().equals("")&&!f_confirmation.getText().equals("")){
			...
			if(f_motDePasse.getText().length()>=parametres.getLongueurmdp()){/**si la longueur du mot de passe est acceptable**/
				if(userService.egaliteMotDePasse(f_motDePasse.getText(), f_confirmation.getText())){
					if(!userService.verifierExistenceMotDePasse(loggedInUser.getIdutilisateur(), f_motDePasse.getText())){/**Verification de l'existence du mot de passe entr�**/
						Utilisateur utilisateur = new Utilisateur();
						utilisateur.setIdutilisateur(loggedInUser.getIdutilisateur());/***renseignement de l'identifiant de l'utillisateur*/
