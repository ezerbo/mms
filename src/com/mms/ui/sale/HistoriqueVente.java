...
public class HistoriqueVente extends JDialog {
	...
	public HistoriqueVente() {
		...
		labelTitre = new JLabel("<html>Ventes realis�es<br>Liste de toutes le vente enregistr�es </html>");
		...
		panelDateDebut.add(new JLabel("D�but : "));
	...
	private void afficheListeClient() {
		LinkedList<Client> listeClient = salesService.listeClient();
	...
	private void venteParametree(String designation) {
		...
		LinkedList<Lignedevente> ligneDeVente = salesService.retourneHistoriqueVente(choixDateDebut.getDate(),choixDateFin.getDate());
	...
	private void abonne() {
		itemTouteLaVente.addActionListener(arg0 -> {
		...
		boutonRechercher.addActionListener(arg0 -> {
		...
		boutonEffectuerRecherche.addActionListener(arg0 -> {
		...
		boutonValider.addActionListener(arg0 -> {
			tableModel.supprimerToutesLesLignes();
			afficheHistoriqueVente(choixDateDebut.getDate(), choixDateFin.getDate());
			popupMenu.remove(itemImprimerFacture);
		});
		comboListeClient.addActionListener(arg0 -> {

		});
		comboListeDesignation.addActionListener(arg0 -> {
	...
	private void afficheHistoriqueVente(Date dateDebut,Date dateFin) {
		LinkedList<Lignedevente> listeVente = salesService.retourneHistoriqueVente(dateDebut,dateFin);
		double montantHT = 0.;
		double montantTVA = 0.;
		double montantTTC = 0.;
	...
	private void effectuerRecherche(int numeroVente){
			LinkedList<Lignedevente> vente = salesService.retournerVente(numeroVente);
			try{
				if(vente.get(0)!=null){
